...
function App() {
  ...
  async function submitProposal(event: React.FormEvent<HTMLFormElement>) {
    ...
    setProposalError('')
    setNotice('')
    if (!proposalForm.teamId || !proposalForm.idea.trim() || !proposalForm.plan.trim() || !proposalForm.timeline.trim() || !proposalForm.prototypeUrl.trim()) {
      setProposalError('Заполните команду, идею, план, срок и ссылку на прототип.')
      return
    }
        ...
        {workspace === 'business' ? (
          ...
          <section className="catalog" aria-labelledby="catalog-title">
            ...
            <div className="catalog__columns">
              ...
              {selectedTask && <article className="task-detail"><div className="task-detail__head"><span className="eyebrow">КАРТОЧКА ЗАДАЧИ</span><ScoreRing score={selectedTask.score} compact /></div><h2>{selectedTask.title || 'Задача без названия'}</h2><p className="task-detail__meta">{selectedTask.industry || 'Отрасль не указана'} · {selectedTask.topic || 'Без темы'}</p><p className="task-detail__summary">{selectedTask.description}</p><div className="task-detail__facts">{(['context', 'need', 'users', 'dataMaterials', 'constraints', 'expectedResult', 'successCriteria', 'contact', 'interaction'] as CardField[]).map((field) => <div key={field}><span>{fieldLabels[field]}</span><p>{selectedTask[field]?.trim() || 'Пока не указано'}</p></div>)}</div><div className="task-detail__bottom"><span className={`readiness readiness--${selectedTask.readiness}`}>{readinessLabels[selectedTask.readiness]}</span><p>Даже при низком рейтинге команда может отправить предложение.</p></div><form className="proposal-form" onSubmit={(event) => void submitProposal(event)}><div className="section-heading"><span className="section-heading__number">↗</span><div><h3>Предложить решение</h3><p>Опишите подход. Выбор команды останется за бизнесом.</p></div></div><label className="field"><span className="field__label">Ваша команда *</span><select value={proposalForm.teamId} required onChange={(event) => setProposalForm((current) => ({ ...current, teamId: event.target.value }))}><option value="">Выберите команду</option>{teams.map((team) => <option value={team.id} key={team.id}>{team.name}</option>)}</select></label><Field label="Идея решения" required value={proposalForm.idea} onChange={(value) => setProposalForm((current) => ({ ...current, idea: value }))} hint="Как вы предлагаете решить задачу?" /><Field label="План работы" required value={proposalForm.plan} onChange={(value) => setProposalForm((current) => ({ ...current, plan: value }))} hint="Основные этапы работы" /><div className="two-columns"><TextInput label="Срок" required value={proposalForm.timeline} onChange={(value) => setProposalForm((current) => ({ ...current, timeline: value }))} placeholder="Например, 2 недели" /><TextInput label="Ссылка на прототип" required type="url" value={proposalForm.prototypeUrl} onChange={(value) => setProposalForm((current) => ({ ...current, prototypeUrl: value }))} placeholder="https://…" /></div>{proposalError && <p className="error-message" role="alert">{proposalError}</p>}<button className="button button--accent" disabled={proposalBusy} type="submit">{proposalBusy ? 'Отправляем…' : 'Отправить предложение'} <ArrowIcon /></button></form></article>}
            </div>
          </section>
        )}
      </main>
