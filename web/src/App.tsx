...
function App() {
  ...
  async function analyze() {
    ...
    try {
      ...
      setSuggestions((current) => {
        ...
        return [...current, ...incoming.filter((item) => !current.some((old) => old.field === item.field))]
      })
  ...
  function acceptSuggestion(suggestion: SuggestedField) {
    ...
    setSuggestions((current) => current.filter((item) => item.field !== suggestion.field))
      ...
      <main className="main">
        <div className="page-heading">
          <div>
            ...
            <h1>{workspace === 'business' ? businessPage === 'responses' ? 'Отклики команд' : 'Создать задачу' : studentPage === 'applications' ? 'Следите за своими откликами' : 'Найдите задачу для своей команды'}</h1>
        ...
        {workspace === 'business' ? (
          ...
          <section className={`catalog ${mobileDetailOpen && selectedTask ? 'catalog--detail-open' : ''}`} aria-labelledby="catalog-title">
            ...
            <nav className="student-tabs" aria-label="Списки задач"><button className="button button--outline" aria-pressed={!savedOnly} onClick={() => { setSavedOnly(false); setMobileDetailOpen(false) }}>Все задачи</button><button className="button button--outline" aria-pressed={savedOnly} onClick={() => { setSavedOnly(true); setMobileDetailOpen(false) }}>Сохранённые ({bookmarks.ids.length})</button></nav>
            ...
            <div className="catalog__columns" aria-busy={catalogLoading}>
              <div className="task-list">{visibleTasks.map((item) => <div className="task-tile-group" key={item.id}><button type="button" id={`task-tile-${item.id}`} key={item.id} className={`task-tile ${selectedTask?.id === item.id ? 'is-selected' : ''}`} onClick={() => openTask(item.id)} aria-pressed={selectedTask?.id === item.id}><div className="task-tile__meta"><span>{item.industry || 'Отрасль не указана'}</span><span>{item.topic || 'Без темы'}</span></div><h3>{item.title || 'Задача без названия'}</h3><p>{item.need || item.description || 'Описание появится после уточнения.'}</p><TaskSummary task={item} />{catalogSort === 'recommended' && recommendationMap.get(item.id)?.reasons.map((reason) => <span className="recommendation-reason" key={reason}>{reason}</span>)}<div className="task-tile__foot"><span className={`readiness readiness--${item.readiness}`}>{readinessLabels[item.readiness]}</span><strong>{item.score}<small>/100</small></strong></div></button><button type="button" className="button button--text bookmark-button" aria-pressed={bookmarks.ids.includes(item.id)} aria-label={`${bookmarks.ids.includes(item.id) ? 'Убрать из сохранённых' : 'Сохранить задачу'}: ${item.title}`} disabled={bookmarks.loading || bookmarks.busy || !!bookmarks.error} onClick={() => void bookmarks.toggle(item.id).then((changed) => { if (changed) recommendations.refresh() })}>{bookmarks.ids.includes(item.id) ? '★ Сохранено · убрать' : '☆ Сохранить задачу'}</button><button type="button" className="button button--text" disabled={recommendations.feedbackBusy || recommendations.loading} onClick={() => void recommendations.dismiss(item.id, !recommendationMap.get(item.id)?.dismissed)}>{recommendationMap.get(item.id)?.dismissed ? 'Вернуть в рекомендации' : 'Не интересно'}</button></div>)}</div>
