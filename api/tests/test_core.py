...
def test_scoring_readiness_band_boundaries():
    assert [readiness_for_score(n) for n in (0, 39, 40, 69, 70, 89, 90, 100)] == [
        "draft", "draft", "workable", "workable", "ready", "ready", "priority", "priority"
    ]
...
def test_seed_is_idempotent(tmp_path):
    with client(tmp_path) as c:
        ...
        assert c.post("/api/admin/seed").status_code == 200
    ...
    with client(tmp_path) as c:
        ...
        for team in teams[:2]:
            response = c.post(
                ...
                json={"teamId": team["id"], "idea": "Reuse delivery totes", "plan": "Test a route", "timeline": "Two weeks", "prototypeUrl": "https://example.org/demo"},
            )
            assert response.status_code == 201
            proposals.append(response.json())
        assert len(c.get(f"/api/tasks/{task_id}/proposals").json()) == 2
        for proposal, decision in zip(proposals, ("selected", "rejected")):
...
def test_low_score_confirmed_task_can_be_published_and_proposed_to(tmp_path):
    with client(tmp_path) as c:
        low = full_card(context="", need="", users="", dataMaterials="", constraints="", expectedResult="", successCriteria="", contact="", interaction="")
        ...
        response = c.post(f"/api/tasks/{task['id']}/proposals", json={"teamId": team["id"], "idea": "An idea", "plan": "A plan", "timeline": "One month", "prototypeUrl": ""})
        assert response.status_code == 201
...
def test_catalog_filters_and_sorts_published_cards(tmp_path):
    with client(tmp_path) as c:
        ...
        topic = rows[0]["topic"]
        assert all(row["topic"] == topic for row in c.get("/api/tasks", params={"topic": topic}).json())
        readiness = rows[0]["readiness"]
        assert all(row["readiness"] == readiness for row in c.get("/api/tasks", params={"readiness": readiness}).json())


def test_analyze_has_three_questions_and_error_envelopes(tmp_path):
    with client(tmp_path) as c:
        result = c.post("/api/analyze", json={"description": "Need a better delivery process", "industry": "Retail"})
        assert result.status_code == 200
        ...
        invalid = c.post("/api/tasks", json={"title": ""})
...
def test_validation_rejects_oversize_and_non_http_urls(tmp_path):
    with client(tmp_path) as c:
        ...
        team = c.get("/api/teams").json()[0]
        bad_url = c.post(f"/api/tasks/{created['id']}/proposals", json={"teamId": team["id"], "idea": "idea", "plan": "plan", "timeline": "now", "prototypeUrl": "javascript:alert(1)"})
