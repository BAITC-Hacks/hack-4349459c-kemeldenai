...
def test_published_edit_requires_atomic_confirmation(client):
    ...
    changed = client.post(
        f"/api/tasks/{task_id}/confirm", json={"title": "Подтверждённая правка"}
    ).json()
    ...
    assert any(
        row["id"] == task_id
        for row in client.get("/api/tasks", params={"readiness": "draft"}).json()
    )
...
def test_same_team_can_submit_repeatedly_and_select_multiple(client):
    ...
    for proposal in (first.json(), second.json()):
        ...
        assert (
            client.patch(
                f"/api/proposals/{proposal['id']}", json={"decision": "selected"}
            ).status_code
            == 200
        )
...
def test_ai_card_values_are_bounded_and_filter_validated(client):
    response = client.post(
        "/api/analyze", json={"description": "Нужен сервис", "card": {"context": "x" * 12001}}
    )
    assert response.status_code == 422
...
def test_openapi_uses_contract_responses_and_camel_case(client):
    ...
    assert schema["components"]["schemas"]["TaskCard"]["properties"].keys() >= {
        "confirmedAt",
        "dataMaterials",
        "createdAt",
        "successCriteria",
...
def test_unknown_resources_and_invalid_decisions(client):
    ...
    for method, path, body in [
        ...
        assert set(response.json()) == {"error"}
    assert (
        client.patch(f"/api/proposals/{missing}", json={"decision": "pending"}).status_code == 422
    )
